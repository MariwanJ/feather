...
namespace feather
{
    ...
    {
        // clear scenegraph
        void FEATHER_API  clear();

        // update scenegraph
        void FEATHER_API  update();

        // get the low uid value
        unsigned int FEATHER_API get_min_uid();

        // get the highest uid value
        unsigned int FEATHER_API get_max_uid();

        // is uid already in scenegraph
        bool FEATHER_API node_exist(unsigned int uid);

        // add a node to the scenegraph
        unsigned int FEATHER_API add_node(unsigned int nid, std::string name, status& error);

        // remove node from scenegraph
        void FEATHER_API  remove_node(unsigned int uid, status& error);

        // get a list of all nodes connected to this node's out fields
        void FEATHER_API  get_node_out_connections(unsigned int uid, std::vector<unsigned int>& uids); 

        // get all the nodes in the scenegraph
        void FEATHER_API  get_nodes(std::vector<unsigned int>& uids);

        // get uid of node by it's name
        void FEATHER_API  get_node_by_name(std::string name, unsigned int& uid);

        // get all nodes of a certain type
        void FEATHER_API  get_nodes_by_type(node::Type type, std::vector<unsigned int>& uids);

        // get all nodes of a certain id 
        void FEATHER_API  get_nodes_by_id(unsigned int id, std::vector<unsigned int>& uids);

        // get node name
        void FEATHER_API  get_node_name(unsigned int uid, std::string& name, status& error);
        ...
        void FEATHER_API  select_node(unsigned int uid, unsigned int fid, unsigned int type, std::vector<unsigned int> ids);
